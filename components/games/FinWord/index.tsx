...
const WORDS = [
  ...
  {
    word: 'LOAN',
    def: 'Money someone lends you that you promise to pay back — usually with extra on top.',
  },
  {
    word: 'DEBT',
    def: 'Money you owe. The longer you take to pay it off, the more it usually costs.',
  },
  {
    word: 'EARN',
    def: 'Money coming in — from a job, a side hustle, selling something, or investments.',
  },
  {
    word: 'RISK',
    def: 'The chance something goes wrong with your money, investment, or property.',
  },
  // 5-letter
  { word: 'ASSET', def: 'Anything you own that has value — cash, car, house, investments.' },
  {
    word: 'CLAIM',
    def: 'A request you make to your insurance company after something goes wrong.',
  },
  {
    word: 'YIELD',
    def: 'How much return an investment gives you — usually shown as a percentage per year.',
  },
  {
    word: 'AGENT',
    def: 'A licensed person who sells and manages insurance policies on your behalf.',
  },
  {
    word: 'SPEND',
    def: 'Using money to buy something. The opposite of saving — not always a bad thing.',
  },
  {
    word: 'TRUST',
    def: 'A legal arrangement where someone manages money or assets for another person.',
  },
  {
    word: 'DEBIT',
    def: 'A card that spends money you already have — directly from your bank account.',
  },
  // 6-letter
  {
    word: 'BUDGET',
    def: "A plan for how you'll spend your money — so you stay in control, not your bank account.",
  },
  {
    word: 'CREDIT',
    def: 'Money you borrow now and pay back later. Good credit history = lower rates on everything.',
  },
  {
    word: 'INVEST',
    def: 'Putting money to work so it grows over time — stocks, funds, real estate.',
  },
  {
    word: 'INCOME',
    def: 'All the money coming in — wages, tips, interest, freelance, side hustle revenue.',
  },
  {
    word: 'EQUITY',
    def: 'What you actually own after subtracting debt. House worth $300K with $200K mortgage = $100K equity.',
  },
  {
    word: 'POLICY',
    def: 'A written agreement between you and an insurance company. Always read the fine print.',
  },
  {
    word: 'INSURE',
    def: 'To protect yourself from financial loss by paying into a plan that covers the unexpected.',
  },
  {
    word: 'REFUND',
    def: 'Money returned to you — from overpaid taxes, a returned item, or a cancelled subscription.',
  },
]
...
export function FinWord() {
  ...
  const submitGuess = useCallback(() => {
    ...
    if (guessStr === word) {
      setGameOver(true)
      const earned = Math.max(50, 200 - currentRow * 25) - (hintUsed ? 50 : 0)
      setXp((x) => x + earned)
      fetch('/api/xp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ gameType: 'fin_word', xpEarned: earned, reason: 'FinWord win' }),
      }).catch(() => {})
      setTimeout(() => {
    ...
    if (nextRow >= maxRows) {
      ...
      fetch('/api/xp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ gameType: 'fin_word', xpEarned: lossXp, reason: 'FinWord attempt' }),
      }).catch(() => {})
      setTimeout(() => {
  ...
  }, [gameOver, currentGuess, wordLen, word, currentRow, hintUsed, maxRows, entry.def])
  ...
  function useHint() {
    if (xp < 50) {
      setMessage('Not enough XP for a hint!', 'error')
      return
    }
    if (hintUsed) {
      setMessage('Hint already used this round!', 'error')
      return
    }
    if (gameOver) return
  ...
  function playAgain() {
    ...
    setEntry(newEntry)
    setBoard(emptyBoard(newEntry.word.length))
    setCurrentRow(0)
    setCurrentGuess([])
    setGameOver(false)
    setKeyStates({})
  ...
  const msgColor = { info: 'text-default-400', error: 'text-red-400', success: 'text-emerald-400' }[
    msg.type
  ]
  const modalEmojis = ['🎉', '🔥', '💡', '⭐', '🏆']
    ...
    <div className={cn('min-h-screen flex flex-col bg-background', spaceGrotesk.className)}>
      ...
      <div className="flex-1 grid grid-cols-[1fr_360px]">
        ...
        <div className="flex flex-col items-center border-r border-white/10 px-8 py-6 gap-3">
          ...
          <div
            className={cn(
              'text-xs font-semibold text-default-400 tracking-widest uppercase',
              spaceMono.className
            )}
          >
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        !tile.revealed &&
                          !tile.result &&
                          (letter ? 'clay-card border-white/30' : 'border-white/10 bg-white/5'),
                        letter && !tile.revealed && styles.pop,
                        hinted && 'border-amber-400/60 text-amber-400',
                        tile.revealed &&
                          tile.result === 'correct' &&
                          cn(
                            'border-emerald-500 bg-emerald-900/60 text-emerald-400',
                            styles.reveal
                          ),
                        tile.revealed &&
                          tile.result === 'present' &&
                          cn('border-amber-500 bg-amber-900/60 text-amber-400', styles.reveal),
                        tile.revealed &&
                          tile.result === 'absent' &&
                          cn('border-white/10 bg-white/10 text-white/40', styles.reveal)
                      )}
                    >
                      {letter}
                    </div>
        ...
        <div className="flex flex-col gap-5 p-6 overflow-y-auto">
          ...
          <div className="clay-card p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={cn('text-xs font-bold text-default-400', spaceMono.className)}>
                ⭐ {xp} XP
              </span>
            </div>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          k.length > 1
                            ? 'min-w-[46px] flex-[1.5] max-w-[58px] text-[10px]'
                            : 'min-w-[26px] flex-1 max-w-[36px]',
                          !state && 'clay-card text-foreground hover:shadow-md',
                          state === 'correct' &&
                            'bg-emerald-900/80 border border-emerald-500 text-emerald-400',
                          state === 'present' &&
                            'bg-amber-900/80 border border-amber-500 text-amber-400',
          ...
          <div className="clay-card p-4 flex flex-col gap-2.5">
            <div className="text-xs font-bold text-default-400 uppercase tracking-widest mb-1">
              Color guide
            </div>
            {[
          ...
          <div className="clay-card p-8 w-full max-w-lg">
            <div className="flex items-start gap-6">
              ...
              <div className="text-center shrink-0">
                ...
                <div
                  className={`text-2xl font-bold ${modal.won ? 'text-emerald-400' : 'text-default-400'}`}
                >
                  +<span className={modal.won ? styles.xpPop : ''}>{modal.earned}</span>
              ...
              <div className="flex-1">
                <div className={cn('text-2xl font-bold text-foreground mb-1', spaceMono.className)}>
                  {modal.won
                    ? modalTitles[Math.min(modal.row, modalTitles.length - 1)]
                    : 'Better luck next time!'}
                </div>
                <div
                  className={cn('text-3xl text-red-500 tracking-[6px] mb-3', spaceMono.className)}
                >
                  {word}
                </div>
