...
const ROUNDS = [
  {
    ...
    pairs: [
      ...
      {
        term: 'SAVINGS',
        def: 'Money you keep instead of spending — your future self will thank you',
      },
  ...
  {
    label: 'Round 2 · Banking & credit',
    color: '#2563eb',
    pairs: [
      {
        term: 'CREDIT SCORE',
        def: 'A number from 300–850 that shows lenders how trustworthy you are with money',
      },
      { term: 'DEBIT CARD', def: 'A card that spends money you already have — not borrowed' },
      {
        term: 'APR',
        def: 'The yearly cost of borrowing money — the higher it is, the more you pay back',
      },
      {
        term: 'OVERDRAFT',
        def: 'Spending more than you have — your bank covers it, then charges you for it',
      },
      {
        term: 'COMPOUND INTEREST',
        def: 'Earning interest on your interest — how $1,000 quietly becomes $2,000 over time',
      },
      {
        term: 'MINIMUM PAYMENT',
        def: "The smallest amount you can pay on a credit card — but you'll owe more next month",
      },
    ],
  },
  {
    label: 'Round 3 · Insurance',
    color: '#7c3aed',
    pairs: [
      {
        term: 'PREMIUM',
        def: 'The monthly amount you pay to keep insurance — like a subscription for protection',
      },
      {
        term: 'DEDUCTIBLE',
        def: 'What you pay first before insurance covers the rest — like an entry fee for a claim',
      },
      {
        term: 'CLAIM',
        def: 'A formal request to your insurer after something goes wrong — how you access your coverage',
      },
      {
        term: 'COVERAGE',
        def: "What your policy actually protects — know what's in before you need it",
      },
      {
        term: 'BENEFICIARY',
        def: 'The person who receives the payout from a life insurance policy',
      },
      {
        term: 'UNDERWRITING',
        def: 'How insurers decide what to charge you — based on your personal risk level',
      },
    ],
  },
]
...
export function TermMatch({ onComplete }: { onComplete?: (summary: string) => void } = {}) {
  ...
  const checkMatch = useCallback(
    (term: string, def: string) => {
      ...
      if (isMatch) {
        ...
        setMatchedInRound(matchedCountRef.current)
  ...
  function cardClass(value: string, isTerm: boolean) {
    ...
    return cn(
      ...
      st === 'selected' &&
        'clay-card ring-2 ring-blue-500 dark:ring-blue-400 bg-blue-50/80 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 -translate-y-0.5',
      st === 'matched' &&
        cn(
          'clay-card-success text-emerald-600 dark:text-emerald-400 cursor-default',
          styles.matchPop
        ),
      st === 'wrong' && cn('clay-card-danger text-red-600 dark:text-red-400', styles.wrongShake),
      st === 'failed' &&
        'border-red-500/15 bg-red-500/5 text-red-300/30 cursor-not-allowed line-through'
    )
  }
        ...
        <div className="max-w-6xl mx-auto px-8 py-20 grid grid-cols-[1fr_440px] gap-16 items-center min-h-[calc(100vh-65px)]">
          ...
          <div>
            ...
            <p className="text-default-400 text-lg leading-relaxed mb-8 max-w-md">
              Tap a term, then tap its definition. Match all pairs to advance. Build combos for
              bonus XP — and hear each term explained when you get it right.
            </p>
            <button
              onClick={startGame}
            ...
            {[
              {
                round: '01',
                label: 'Money basics',
                desc: '6 fundamental terms everyone should know',
                color: '#CC0000',
              ...
              },
              {
                ...
                desc: 'Credit scores, APR, compound interest and more',
                color: '#2563eb',
              ...
              },
              {
                round: '03',
                label: 'Insurance',
                desc: 'Premiums, claims, deductibles — how coverage works',
                color: '#7c3aed',
              ...
              <div
                key={c.round}
                className={cn('clay-card p-5 flex items-center gap-5 border', c.border, c.bg)}
              >
                <div className="text-3xl font-black shrink-0" style={{ color: c.color }}>
                  {c.round}
                </div>
                <div>
          ...
          {showRoundBanner ? (
            ...
            <>
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  <div className="text-xs font-black text-default-400 uppercase tracking-widest mb-1">
                    Now playing
                  </div>
                  <div
                    className="text-2xl font-black text-foreground"
                    style={{ color: ROUNDS[round]?.color }}
                  >
                    {ROUNDS[round]?.label}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  {matchStreak >= 2 && (
                    <div className="clay-card px-3 py-1 text-xs font-bold text-amber-500">
                      🔥 {matchStreak} streak
                    </div>
                  )}
                  <div className="clay-card px-3 py-1 text-xs font-bold text-default-600">
                    ⭐ {xp} XP
                  </div>
                  <div
                    className="clay-card px-4 py-2 text-sm font-black"
                    style={{ color: ROUNDS[round]?.color }}
                  >
                    ...
                    {Array.from({ length: pairsInRound }, (_, i) => (
                      <div
                        ...
                        className={cn(
                          ...
                          i < matchedInRound ? 'bg-emerald-400' : 'bg-white/15'
                        )}
                      />
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                {/* TERMS */}
                <div>
                  <div className="text-xs font-black uppercase tracking-widest text-default-400 mb-3">
                    Terms
                  </div>
                    ...
                    {terms.map((term) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          if (st !== 'matched' && st !== 'wrong' && st !== 'failed')
                            selectCard(term, 'term')
                        }}
                      >
                        {term}
                    ...
                    {defs.map((def) => (
                      ...
                      >
                        ...
                        {cardStates[def] === 'matched' && (
                          <span className="absolute right-4 text-base text-emerald-400 shrink-0">
                            ✓
                          </span>
                        )}
                      </div>
        ...
        <div className="max-w-6xl mx-auto px-8 py-10 h-[calc(100vh-52px)] flex flex-col">
          <div className="grid grid-cols-[1fr_440px] gap-10 flex-1 min-h-0">
            ...
            <div className="flex flex-col justify-center">
              ...
              <div className="text-default-400 text-base mb-8">
                {totalMatched} of {totalPairs} pairs matched
              </div>
              <div className="grid grid-cols-3 gap-3 mb-8">
                {[
                  { val: xp, lbl: 'Total XP', color: 'text-red-500' },
                  { val: totalMatched, lbl: 'Matched', color: 'text-emerald-500' },
                  { val: bestStreak, lbl: 'Best streak', color: 'text-amber-500' },
                ].map((s) => (
                  <div key={s.lbl} className="clay-card p-5 text-center">
                    <div className={cn('text-3xl font-black', s.color)}>{s.val}</div>
                    <div className="text-xs text-default-400 font-bold mt-1 uppercase tracking-wide">
                      {s.lbl}
                    </div>
                  </div>
                ))}
              ...
              <div className="flex-1 overflow-y-auto divide-y divide-white/10 pr-1">
                {allPairs.map((p) => (
                  <div key={p.term} className="py-2.5 flex gap-4">
                    <span className="font-black text-red-500 shrink-0 text-xs w-32 pt-0.5">
                      {p.term}
                    </span>
