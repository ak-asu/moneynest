...
type GameEvent = {
  ...
  source: string
  lesson: string
  effects: Partial<Record<AssetId, number>>
  color: string
}
...
const EVENT_POOL: GameEvent[] = [
  {
    ...
    lesson:
      'The S&P 500 fell 38.5% in 2008. Diversified investors recovered by 2013. Those who sold at the bottom never did.',
    effects: { stock: -0.35, index: -0.2, realty: -0.15, bonds: 0.05 },
  },
  {
    icon: '🌧️',
    title: 'Fed Rate Hike Cycle',
    color: '#f87171',
    ...
    lesson:
      'The Fed raised rates 7× in 2022 to fight inflation. When borrowing costs rise, growth stocks fall hardest — and older bonds lose value too.',
    effects: { stock: -0.19, index: -0.15, bonds: -0.1, savings: 0.015 },
  },
  {
    icon: '🌞',
    ...
    lesson:
      'Markets recovered in 2023 — the S&P 500 gained 24%. Staying invested through downturns is how long-term investors win.',
    effects: { stock: 0.35, index: 0.2 },
  },
  {
    icon: '🏡',
    ...
    voice:
      'Real estate boom. US home prices rose nearly 19 percent. Property investors are winning big.',
    lesson:
      'US home prices rose 18.8% in 2021 per Case-Shiller. Low interest rates and remote work demand created a historic housing boom.',
    effects: { realty: 0.18, stock: 0.1 },
  },
  {
    icon: '📈',
    ...
    voice:
      'Recovery rally. The S&P 500 gained 26 percent. The biggest gains come right after the panic.',
    lesson:
      'After the 2008 crash, the S&P 500 gained 26.5% in 2009. The biggest gains often come right after the biggest panic.',
    effects: { stock: 0.25, index: 0.15 },
  },
  {
    icon: '🌾',
    title: 'Sustained Bull Market',
    color: '#4ade80',
    ...
    lesson:
      'Index fund investors who held 2013–2021 turned $1,000 into $4,200. Time in the market beats timing the market.',
    effects: { index: 0.1, stock: 0.12, realty: 0.08 },
  },
  {
    icon: '🌪️',
    ...
    lesson:
      'US inflation hit 8.5% in 2022 — a 40-year high. Cash and bonds lose purchasing power. Real assets like property hold up.',
    effects: { savings: -0.04, bonds: -0.06, realty: 0.1 },
  },
  {
    icon: '🤖',
    title: 'AI & Tech Rebound',
    color: '#60a5fa',
    source: 'Based on 2023',
    ...
    lesson:
      "The NASDAQ gained 43% in 2023 driven by AI growth. High-risk assets can recover sharply — but only if you didn't sell during the crash.",
    effects: { stock: 0.38, index: 0.15 },
  },
  {
    icon: '🏦',
    ...
    lesson:
      'When SVB collapsed in 2023, scared investors moved money into bonds and savings accounts. Diversification across asset types protects against sector shocks.',
    effects: { stock: -0.12, bonds: 0.06, savings: 0.02, index: -0.06 },
  },
  {
    icon: '⚡',
    title: 'Energy Boom',
    ...
    voice:
      'Energy boom. Global energy prices surged. Real assets and inflation hedges are winning.',
    lesson:
      'Energy prices doubled in 2021-2022. Commodities and real estate often rise with inflation, showing why not all assets move together.',
    effects: { realty: 0.12, savings: 0.01, stock: 0.08, bonds: -0.03 },
  },
  {
    icon: '🎯',
    title: 'IPO Frenzy',
    ...
    voice:
      'IPO frenzy. Record new companies went public. Tech and index funds surged on investor optimism.',
    lesson:
      'In 2021, over 1,000 companies went public. Hype-driven markets can lift all assets — but they also set the stage for sharp corrections.',
    effects: { stock: 0.22, index: 0.12 },
  },
  {
    icon: '🦠',
    title: 'COVID Crash',
    ...
    lesson:
      'In 2020 the S&P 500 crashed 34% in 33 days, then fully recovered in just 5 months. Panic selling locked in losses. Staying in captured the rebound.',
    effects: { stock: -0.3, index: -0.25, realty: -0.08, bonds: 0.04 },
  },
  {
    icon: '💻',
    ...
    voice:
      'Dot-com bubble burst. The NASDAQ lost 78 percent. Diversification was the only shelter.',
    lesson:
      'The NASDAQ fell 78% from 2000–2002 as internet valuations collapsed. Concentrated tech bets were wiped out. Diversification across asset classes cushioned the blow.',
    effects: { stock: -0.4, index: -0.22, bonds: 0.08, savings: 0.02 },
  },
  {
    icon: '🚀',
    ...
    lesson:
      'After the 2020 crash, the S&P 500 gained 100% in 18 months. Easy money and fiscal stimulus created one of the fastest bull markets ever recorded.',
    effects: { stock: 0.4, index: 0.28, realty: 0.2, bonds: -0.03 },
  },
  {
    icon: '🏛️',
    ...
    voice:
      'Government stimulus. Massive spending propped up markets. Safety assets led the recovery.',
    lesson:
      'Both the 2009 and 2020 recoveries were turbocharged by government stimulus. Understanding policy impact is as important as understanding markets.',
    effects: { bonds: 0.08, realty: 0.1, index: 0.12, savings: 0.01 },
  },
]
...
export function WealthFarm() {
  ...
  const tick = useCallback(() => {
    ...
    const { portfolio: next, event } = applyYear(
      portfolioRef.current,
      nextYear,
      gameEventsMapRef.current
    )
    setPortfolio(next)
    setSimYear(nextYear)
    ...
    if (nextYear === 5) {
      const rounded = ASSETS.reduce(
        (acc, a) => {
          acc[a.id] = Math.round(next[a.id])
          return acc
        },
        {} as Record<AssetId, number>
      )
      const diff =
        Math.round(next.savings + next.index + next.stock + next.bonds + next.realty) -
        Object.values(rounded).reduce((a, b) => a + b, 0)
      if (diff !== 0) {
        const adjust = ASSETS.map((a) => ({
          id: a.id,
          frac: next[a.id] - Math.floor(next[a.id]),
    ...
    if (nextYear >= 10) {
      ...
      setTimeout(() => {
        ...
        fetch('/api/xp', {
          ...
          body: JSON.stringify({
            gameType: 'wealth_farm',
            xpEarned: xp,
            reason: 'WealthFarm completed',
          }),
        }).catch(() => {})
      }, 1800)
    }
  ...
  const rebalanceAllocTotal = ASSETS.reduce(
    (s, a) => s + (parseFloat(rebalanceAlloc[a.id]) || 0),
    0
  )
...
Keep it friendly, educational, and specific to their results. No markdown, just plain text.`
  ...
  const bestAsset = ASSETS.reduce(
    (best, a) => (portfolio[a.id] > portfolio[best.id] ? a : best),
    ASSETS[0]
  )
  ...
  const SP500_10YR = Math.round(1000 * Math.pow(1.1, 10)) // $2,594
      ...
      {phase === 'intro' && (
        <div className="max-w-7xl mx-auto px-8 py-20 grid grid-cols-[1fr_520px] gap-16 items-center min-h-[calc(100vh-65px)]">
          ...
          <div>
            ...
            <h1
              className={cn(
                'text-5xl font-bold text-foreground leading-tight mb-6',
                pixelify.className
              )}
            >
              Grow your <span className="text-emerald-400">$1,000</span>
              <br />
              into something
              <br />
            ...
            <p className="text-default-400 text-lg leading-relaxed mb-8 max-w-md">
              Plant your money into different investments. Watch real market events unfold over 10
              years. At Year 5, decide if you want to rebalance. At Year 10, see what you built.
            </p>
            <button
              onClick={() => setPhase('game')}
              className={cn(
                'clay-btn bg-emerald-700 hover:bg-emerald-600 text-white px-12 py-5 text-xl font-bold rounded-2xl transition-all hover:-translate-y-1 hover:shadow-xl',
                pixelify.className
              )}
            >
              Start farming →
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {[
                {
                  icon: '📊',
                  text: 'Only 24% of teens pass a basic money literacy test',
                  src: 'CFPB',
                },
                {
                  icon: '📈',
                  text: '$1,000 in the S&P 500 in 2013 = ~$3,200 by 2023',
                  src: 'S&P 500',
                },
                ...
                <div
                  key={s.src}
                  className="clay-card px-3 py-2 text-xs text-default-400 flex items-start gap-2 max-w-[220px]"
                >
                  <span className="shrink-0">{s.icon}</span>
                  <span>
                    {s.text} <span className="text-default-500 italic">— {s.src}</span>
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="max-w-7xl mx-auto w-full px-8 py-6 grid grid-cols-[1fr_380px] gap-6 items-start">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="clay-card px-5 py-4 flex items-center gap-4">
              <span className="text-3xl">
                {isSimulating || simPhase === 'finished' ? season.icon : '🌱'}
              </span>
              <div className="flex-1">
                <div className="font-bold text-amber-400 text-base">
                  {simPhase === 'idle'
                    ? 'Year 1 — Plant your seeds'
                    : `${season.name} · Year ${simYear}`}
                </div>
                <div className="text-sm text-default-500 mt-0.5">
                  {simPhase === 'idle'
                    ? 'Allocate your $1,000 before the simulation starts.'
                    : season.desc}
                </div>
              </div>
            ...
            {currentEvent && (
              <div
                className={cn('clay-card px-5 py-4 border-l-4', styles.fadeIn)}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-bold text-base" style={{ color: currentEvent.color }}>
                        {currentEvent.title}
                      </span>
                      {currentEvent.source && (
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-sm font-semibold">
                    <span className="text-default-400">
                      ${Math.round(preAllocTotal)} allocated ·{' '}
                    </span>
                    <span className={preAllocTotal > 1000 ? 'text-red-400' : 'text-amber-400'}>
                      {preAllocTotal > 1000
                        ? `-$${Math.round(preAllocTotal - 1000)} over`
                        : `$${Math.round(preAllocRemaining)} left`}
                    </span>
                  </div>
                </div>
                    ...
                    <div key={a.id} className="flex items-center gap-4 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-default-500 mt-0.5">
                          {a.source} · <span style={{ color: a.riskColor }}>{a.risk}</span> ·{' '}
                          {Math.round(a.rateMin * 100)}–{Math.round(a.rateMax * 100)}%/yr
                        </div>
                      </div>
                      <div className="relative shrink-0">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-default-400 pointer-events-none">
                          $
                        </span>
                        <input
                          type="number"
            ...
            {isPausedYr5 && (
              <div className="clay-card p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-sm font-semibold">
                    <span className="text-default-400">
                      ${Math.round(rebalanceAllocTotal)} allocated ·{' '}
                    </span>
                    <span
                      className={rebalanceAllocTotal > nwBudget ? 'text-red-400' : 'text-amber-400'}
                    >
                      {rebalanceAllocTotal > nwBudget
                        ? `-$${Math.round(rebalanceAllocTotal - nwBudget)} over`
                        : `$${Math.round(rebalanceAllocRemaining)} unallocated`}
                    </span>
                  </div>
                </div>
                <div className="w-full h-1.5 rounded-full bg-white/10 mb-4 overflow-hidden">
                    ...
                    <div key={a.id} className="flex items-center gap-4 py-3">
                      ...
                      <div className="relative shrink-0">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setRebalanceAlloc((p) => ({ ...p, [a.id]: e.target.value }))
                          }
                          className="clay-input w-28 pl-6 pr-3 py-2 text-sm text-foreground outline-none text-right"
                        />
                      </div>
              ...
              <div className="clay-card border-amber-400/30 px-6 py-5 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm text-default-500">
                    Halfway done. Sell and reinvest above, or continue as-is.
                  </div>
                </div>
                <button
                  onClick={resumeAfterRebalance}
                  disabled={rebalanceAllocTotal > nwBudget}
                  className={cn(
                    'clay-btn bg-emerald-700 hover:bg-emerald-600 disabled:opacity-40 disabled:cursor-not-allowed text-white px-8 py-3 font-bold rounded-2xl transition-all',
                    rebalanceAllocTotal > nwBudget ? dmSans.className : pixelify.className
                  )}
                >
                  {rebalanceAllocTotal > nwBudget
                    ? `Over by $${Math.round(rebalanceAllocTotal - nwBudget)}`
                    : 'Continue to Year 10 →'}
                </button>
              </div>
            )}
            ...
            {simPhase === 'idle' && (
              <button
                onClick={startSim}
                disabled={preAllocTotal > 1000}
                className={cn(
                  'clay-btn bg-amber-600 hover:bg-amber-500 disabled:opacity-40 disabled:cursor-not-allowed text-black px-6 py-4 font-bold rounded-2xl transition-all text-base w-full hover:-translate-y-0.5',
                  preAllocTotal > 1000 ? dmSans.className : pixelify.className
                )}
              >
                {preAllocTotal > 1000
                  ? `Over budget by $${Math.round(preAllocTotal - 1000)}`
                  : '⏩ Start simulation — watch 10 years unfold'}
              </button>
            )}
          </div>
          ...
          <div className="sticky top-[73px] flex flex-col gap-4">
            <div className="clay-card p-5">
              ...
              >
                {change >= 0 ? '+' : ''}${Math.round(Math.abs(change)).toLocaleString()} (
                {change >= 0 ? '+' : ''}
                {changePct}%) vs start
              </div>
              <div className="flex items-center gap-3 px-3 mb-1">
                <div className="text-[10px] text-default-500 uppercase tracking-wide flex-1">
                  Asset
                </div>
                <div className="text-[10px] text-default-500 uppercase tracking-wide w-8 text-right">
                  Alloc
                </div>
                <div className="text-[10px] text-default-500 uppercase tracking-wide w-16 text-right">
                  Value
                </div>
                <div className="text-[10px] text-default-500 uppercase tracking-wide w-12 text-right">
                  Return
                </div>
              </div>
              <div className="flex flex-col gap-2">
                {[
                  { id: 'cash', name: 'Cash', icon: '💵', color: '#fbbf24', value: cash },
                  ...ASSETS.map((a) => ({ ...a, value: portfolio[a.id] })),
                ].map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <div className="text-sm flex-1 text-foreground font-medium">
                        {item.icon} {item.name}
                      </div>
                      <div className="text-xs text-default-400 w-8 text-right">
                      ...
                      <div
                        ...
                        style={{
                          color: gPct === null ? 'transparent' : gain >= 0 ? '#4ade80' : '#f87171',
                        }}
                      >
            ...
            {simPhase !== 'idle' && (
              <div className="clay-card p-4">
                <div className="text-xs font-bold text-default-400 uppercase tracking-widest mb-3">
                  📅 Year history
                </div>
                <div className="flex flex-col gap-1 max-h-64 overflow-y-auto pr-1">
                  {eventLog.length === 0 && (
                    <div className="text-xs text-default-500 italic">
                      Events will appear here as years pass.
                    </div>
                  )}
                  {eventLog.map(({ yr, event }) => (
                    ...
                    >
                      <div className="text-xs font-bold text-default-500 shrink-0 w-12">
                        Yr {yr}
                      </div>
                      {event ? (
        ...
        <div className="max-w-7xl mx-auto px-8 py-16">
          <div className="grid grid-cols-[1fr_480px] gap-12 items-start">
            ...
            <div>
              ...
              <div className={cn('text-5xl font-bold text-emerald-400 mb-2', pixelify.className)}>
                {gradeText}
              </div>
              ...
              <div className="grid grid-cols-4 gap-4 mb-8">
                {[
                  {
                    val: `$${Math.round(nw).toLocaleString()}`,
                    lbl: 'Net worth',
                    color: '#4ade80',
                  },
                  {
                    val: `${change >= 0 ? '+' : ''}${changePct}%`,
                    lbl: 'Total return',
                    color: '#fbbf24',
                  },
                  {
                    val: `${bestAsset.icon} ${bestAsset.name.split(' ')[0]}`,
                    lbl: 'Best crop',
                    color: '#60a5fa',
                  },
                  {
                  ...
                  <div key={s.lbl} className="clay-card p-5 text-center">
                    ...
                    <div className="text-xs text-default-400 uppercase tracking-wide font-semibold">
                      {s.lbl}
                    </div>
                  </div>
                ))}
              ...
              <button
                onClick={restartFarm}
                className={cn(
                  'clay-btn bg-emerald-700 hover:bg-emerald-600 text-white px-10 py-4 text-lg font-bold rounded-2xl transition-all hover:-translate-y-0.5',
                  pixelify.className
                )}
              >
                Play again →
            ...
            <div className="flex flex-col gap-6">
              <div className="clay-card p-6">
                <h3 className={cn('text-sm text-amber-400 mb-4', pixelify.className)}>
                  📊 Final investment split
                </h3>
                <div className="flex flex-col gap-2">
                  {[
                    ...ASSETS.map((a) => ({ ...a, value: portfolio[a.id] })),
              ...
              <div className="clay-card p-6">
                <h3 className={cn('text-sm text-amber-400 mb-5', pixelify.className)}>
                  📖 What the farm taught you
                </h3>
                  ...
                  {[
                    {
                      icon: '🌾',
                      title: 'Boring wins',
                      text: 'The S&P 500 averaged ~10%/yr since 1957. $1,000 invested at 18 becomes ~$452,000 at 65. Start at 30: ~$128,000. The boring choice is the best choice.',
                    },
                    {
                      icon: '💥',
                      title: 'Crashes are real',
                      text: "The 2008 crash erased 38.5% from the S&P 500. Diversified investors recovered fully by 2013. Those who panicked and sold, didn't.",
                    },
                    {
                      icon: '⏰',
                      title: 'Compound interest',
                      text: '$1,000 at 10%/yr becomes $2,594 in 10 years — and $17,449 in 30 years. Every decade of waiting costs you thousands.',
                    },
                    {
                      icon: '🛡️',
                      title: 'Protect what you build',
                      text: 'Building wealth is only half the job. Insurance protects against the unexpected losses that can wipe out years of gains overnight.',
                    },
                  ].map((l) => (
