...
import type {
  DbUser, DbProfile, DbLearningProgress, DbBudgetEntry, DbActionPlan, DbSuggestion,
} from '@/types/database'
...
export async function GET() {
  ...
  const { data } = await (supabase
    ...
    .order('created_at', { ascending: false }) as unknown as Promise<{ data: DbSuggestion[] | null }>)
...
export async function POST() {
  ...
  const [profileRes, learningRes, budgetRes, plansRes] = await Promise.all([
    ...
    supabase.from('budget_entries').select('*').eq('user_id', dbUser.id)
      .order('date', { ascending: false }).limit(30),
    supabase.from('action_plans').select('*').eq('user_id', dbUser.id)
  ...
  const stalePlansCount = activePlans.filter(p => isProfileStale(p.profile_snapshot_hash, profile)).length
  const unmasteredConcepts = learning.filter(l => l.confidence_level !== 'high').map(l => l.concept)
...
- Recent transactions: ${recentBudget.slice(0, 5).map(e => `${e.entry_type} ${e.category} $${e.amount}`).join(', ') || 'none'}
...
[{"type":"insight"|"simulation"|"learning"|"plan","title":"...","reason":"...","severity":"low"|"medium"|"high","chat_seed":"..."}]`
  ...
  type SuggestionInput = { type: string; title: string; reason: string; severity: string; chat_seed: string }
  ...
  await (supabase.from('suggestions') as any).delete().eq('user_id', dbUser.id).eq('dismissed', false)
  ...
  const { data: inserted, error } = await ((supabase.from('suggestions') as any)
    .insert(rows)
    .select() as unknown as Promise<{ data: DbSuggestion[] | null; error: { message: string } | null }>)
...
export async function PATCH(req: Request) {
  ...
  await (supabase.from('suggestions') as any).update({ dismissed: true }).eq('id', id).eq('user_id', dbUser.id)
