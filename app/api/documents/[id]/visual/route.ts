...
function buildImagePrompt(docType: DocumentKind, explanation: DocumentExplanation | null): string {
  ...
  const riskTone =
    highRisk.length >= 5
      ? 'urgent warning atmosphere with red caution symbols'
      : highRisk.length >= 2
        ? 'cautious amber warning mood with alert badges'
        : totalRisk > 0
          ? 'mild advisory feel with subtle caution indicators'
          : 'calm and reassuring safe atmosphere'

  // Key clause labels for visual detail
  const clauseDetails = highRisk
    .slice(0, 2)
    .map((c) => c.label)
    .join(', ')
  ...
  const flagDetail =
    riskFlags.length > 0 ? `Visual elements referencing: ${riskFlags.join('; ')}.` : ''
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const prompt = buildImagePrompt(
    doc.document_type as DocumentKind,
    doc.ai_explanation as DocumentExplanation | null
  )
