...
async function detectAndInsertSpikes(
  ...
  const [currentRes, prevRes] = await Promise.all([
    (supabase.from('budget_entries') as any)
      ...
      .gte('date', d30)
      .lte('date', today) as unknown as Promise<{ data: Pick<DbBudgetEntry, 'category' | 'amount'>[] | null }>,
    (supabase.from('budget_entries') as any)
      .select('category, amount')
      .eq('user_id', userId)
      .eq('entry_type', 'expense')
      .gte('date', d60)
      .lt('date', d30) as unknown as Promise<{ data: Pick<DbBudgetEntry, 'category' | 'amount'>[] | null }>,
  ])
