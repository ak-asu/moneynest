...
import { Plus, Trash2, Save, Wallet, BadgeDollarSign, Target, UserRound, CreditCard } from 'lucide-react'
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  ...
                  <p className="max-w-2xl text-default-500">
                    Shape your money setup in one place. Your identity, cash flow, debts, and goals all feed the advice you get across Vela.
                  </p>
                </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                  <div className="rounded-2xl border border-white/10 bg-default-50/80 p-4">
                    <div className="mb-2 flex items-center gap-2 text-default-500">
                      <UserRound size={16} />
                      <span className="text-xs font-semibold uppercase tracking-[0.18em]">Persona</span>
                    </div>
                    <div className="text-lg font-bold">{selectedPersona?.label ?? 'Other'}</div>
                    <div className="text-sm text-default-500">{language === 'en' ? 'English' : 'Español'}</div>
                  </div>
                  <div className="rounded-2xl border border-emerald-500/15 bg-emerald-500/5 p-4">
                    <div className="mb-2 flex items-center gap-2 text-emerald-300">
                      <BadgeDollarSign size={16} />
                      <span className="text-xs font-semibold uppercase tracking-[0.18em]">Income</span>
                    </div>
                    <div className="text-lg font-bold">{formatMoney(Number(incomeMonthly) || 0)}</div>
                    <div className="text-sm text-default-500">{incomeType === 'steady' ? 'Same every month' : 'Income varies'}</div>
                  </div>
                  <div className="rounded-2xl border border-amber-500/15 bg-amber-500/5 p-4">
                    <div className="mb-2 flex items-center gap-2 text-amber-300">
                      <Wallet size={16} />
                      <span className="text-xs font-semibold uppercase tracking-[0.18em]">Monthly Left</span>
                    </div>
                    <div className="text-lg font-bold">{formatMoney(monthlyLeft)}</div>
                    <div className="text-sm text-default-500">{formatMoney(totalExpenses)} in expenses</div>
                  </div>
                  <div className="rounded-2xl border border-sky-500/15 bg-sky-500/5 p-4">
                    <div className="mb-2 flex items-center gap-2 text-sky-300">
                      <Target size={16} />
                      <span className="text-xs font-semibold uppercase tracking-[0.18em]">Savings</span>
                    </div>
                    <div className="text-lg font-bold">{formatMoney(Number(savingsBalance) || 0)}</div>
          ...
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-12">
            <section className="clay-card p-6 space-y-5 xl:col-span-5">
              <div className="space-y-1">
                <div className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Profile Basics</div>
                <h2 className="text-2xl font-bold">Identity</h2>
                <p className="text-sm text-default-500">Tell Vela who you are so your coaching matches your real situation.</p>
              </div>
            ...
            <section className="clay-card p-6 space-y-5 xl:col-span-7">
              <div className="space-y-1">
                <div className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-400">Cash Flow</div>
                <h2 className="text-2xl font-bold">Income</h2>
                <p className="text-sm text-default-500">Keep your monthly picture current so plans and suggestions stay realistic.</p>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl border border-default-200 bg-default-50/80 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-default-500">Income</div>
                  <div className="mt-2 text-xl font-bold">{formatMoney(Number(incomeMonthly) || 0)}</div>
                </div>
                <div className="rounded-2xl border border-default-200 bg-default-50/80 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-default-500">Expenses</div>
                  <div className="mt-2 text-xl font-bold">{formatMoney(totalExpenses)}</div>
                </div>
                <div className="rounded-2xl border border-default-200 bg-default-50/80 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-default-500">Left This Month</div>
                  <div className="mt-2 text-xl font-bold text-emerald-400">{formatMoney(monthlyLeft)}</div>
                </div>
              </div>
            </section>
            ...
            <section className="clay-card p-6 space-y-5 xl:col-span-7">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-400">Spending</div>
                  <h2 className="text-2xl font-bold">Monthly Expenses</h2>
                  <p className="text-sm text-default-500">List the bills and categories that shape your month-to-month budget.</p>
                </div>
                <Button
                  type="button"
                  size="sm"
              ...
              <div className="space-y-3">
                {expenseFields.map((field, i) => (
                  <div key={field.id} className="grid grid-cols-[minmax(0,1fr)_160px_auto] gap-3 items-start rounded-2xl border border-default-200 bg-default-50/70 p-3">
                    <div>
                      <input
                      ...
                      {errors.expenses?.[i]?.key && (
                        <p className="text-danger text-xs mt-1">{errors.expenses[i].key?.message}</p>
                      )}
                    </div>
                    ...
                    <button type="button" onClick={() => removeExpense(i)} className="mt-1 rounded-xl p-2 text-danger hover:bg-danger-50">
                      <Trash2 size={15} />
                    </button>
                  </div>
                ))}
              ...
              {expenseFields.length === 0 && (
                <div className="rounded-2xl border border-dashed border-default-200 bg-default-50/50 p-6 text-sm text-default-400">
                  No expenses added yet. Add rent, food, transport, subscriptions, and anything else that hits monthly.
                </div>
              )}
            </section>
            ...
            <section className="clay-card p-6 space-y-5 xl:col-span-5">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-rose-400">Obligations</div>
                  <h2 className="text-2xl font-bold">Debts</h2>
                  <p className="text-sm text-default-500">Track balances and rates so payoff suggestions are based on reality.</p>
                </div>
                <Button
                  type="button"
                  size="sm"
              ...
              <div className="rounded-2xl border border-default-200 bg-default-50/70 p-4">
                <div className="mb-1 flex items-center gap-2 text-default-500">
                  <CreditCard size={15} />
                  <span className="text-xs font-semibold uppercase tracking-[0.18em]">Total Debt</span>
                </div>
                <div className="text-2xl font-bold">{formatMoney(totalDebt)}</div>
              </div>
              {debtFields.map((field, i) => (
                <div key={field.id} className="grid grid-cols-[1fr_100px_80px_auto] gap-2 items-start rounded-2xl border border-default-200 bg-default-50/70 p-3">
                  <input
                  ...
                  <button type="button" onClick={() => removeDebt(i)} className="rounded-xl p-2 text-danger hover:bg-danger-50">
                    <Trash2 size={14} />
                  </button>
                </div>
              ))}
              {debtFields.length === 0 && (
                <div className="rounded-2xl border border-dashed border-emerald-500/20 bg-emerald-500/5 p-6 text-sm text-default-300">
                  No debts recorded. That gives you more room to build savings and hit your goals faster.
                </div>
              )}
            </section>
            ...
            <section className="clay-card p-6 space-y-5 xl:col-span-12">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-400">Future Plans</div>
                  <h2 className="text-2xl font-bold">Goals</h2>
                  <p className="text-sm text-default-500">Track what you are saving toward so Vela can keep your plan aligned with it.</p>
                </div>
                <Button
                  type="button"
                  size="sm"
              ...
              <div className="grid gap-4 lg:grid-cols-[260px_minmax(0,1fr)]">
                <div className="rounded-2xl border border-default-200 bg-default-50/70 p-5">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-default-500">Goal Target Total</div>
                  <div className="mt-2 text-3xl font-bold">{formatMoney(totalGoalTarget)}</div>
                  <div className="mt-2 text-sm text-default-500">{goals.length} goal{goals.length === 1 ? '' : 's'} tracked</div>
                </div>
                <div className="space-y-3">
                  {goalFields.map((field, i) => (
                    <div key={field.id} className="grid grid-cols-[minmax(0,1fr)_130px_170px_auto] gap-3 items-start rounded-2xl border border-default-200 bg-default-50/70 p-3">
                      <input
                      ...
                      />
                      <button type="button" onClick={() => removeGoal(i)} className="rounded-xl p-2 text-danger hover:bg-danger-50">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  ))}
                  {goalFields.length === 0 && (
                    <div className="rounded-2xl border border-dashed border-sky-500/20 bg-sky-500/5 p-6 text-sm text-default-300">
                      No goals yet. Add something concrete like an emergency fund, tuition payment, or travel target.
                    </div>
                  )}
                </div>
              </div>
