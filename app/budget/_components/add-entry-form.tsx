...
export function AddEntryForm({ onAdded }: AddEntryFormProps) {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    if (!category.trim() || !amount || !date) { setError('All fields are required.'); return }
    ...
    try {
      const res = await fetch('/api/budget', {
        ...
        body: JSON.stringify({ category: category.trim(), amount: parsed, entry_type: entryType, date }),
      })
  ...
  return (
    <form onSubmit={submit} className="clay-card p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label htmlFor="entry-category" className="text-xs font-medium text-default-600 mb-1 block">Category</label>
          <input
            id="entry-category"
            value={category}
        ...
        <div>
          <label htmlFor="entry-amount" className="text-xs font-medium text-default-600 mb-1 block">Amount ($)</label>
          <input
            id="entry-amount"
            type="number"
            min="0"
        ...
        <div>
          <div role="group" aria-label="Entry type" className="text-xs font-medium text-default-600 mb-1 block">Type</div>
            ...
            {(['income', 'expense'] as EntryType[]).map(t => (
              <button
                ...
                className={`flex-1 rounded-lg border py-1.5 text-xs font-medium transition-colors ${
                  entryType === t
                    ? t === 'income' ? 'bg-success-100 border-success text-success-700' : 'bg-danger-100 border-danger text-danger-700'
        ...
        <div>
          <label htmlFor="entry-date" className="text-xs font-medium text-default-600 mb-1 block">Date</label>
          <input
            id="entry-date"
            type="date"
            value={date}
      ...
      <div className="flex gap-2">
        ...
        <Button type="button" size="sm" variant="ghost" onPress={() => setOpen(false)} className="clay-btn">
          Cancel
        </Button>
      </div>
    </form>
